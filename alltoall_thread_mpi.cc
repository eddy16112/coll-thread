...
{	
  ...
  int sendto_global_rank, recvfrom_global_rank, sendto_mpi_rank, recvfrom_mpi_rank;
    ...
    int send_tag = (sendto_global_rank * 10000 + global_rank) * 10 + ALLTOALL_TAG; // which dst seg it sends to (in dst rank)
    int recv_tag = (global_rank * 10000 + recvfrom_global_rank) * 10 + ALLTOALL_TAG; // idx of current seg we are receving (in src/my rank)
#ifdef DEBUG_PRINT
    ...
    int send_tag = (i * 10000 + global_rank) * 10 + ALLTOALL_TAG; // which seg it sends to
    int recv_tag = (global_rank * 10000 + i) * 10 + ALLTOALL_TAG; // idx of current seg
#ifdef DEBUG_PRINT
    printf("i: %d === global_rank %d, mpi rank %d, send %d to %d, send_tag %d, recv %d from %d, recv_tag %d\n", 
  ...
  int dest_mpi_rank;
    ...
    char* src = (char*)sendbuf_tmp + i * sendtype_extent * sendcount;
    dest_mpi_rank = i / global_comm.nb_threads;
    tag = (i * 10000 + global_rank) * 10 + ALLTOALL_TAG;
  ...
  for(int i = 0 ; i < total_size; i++) {
    ...
    tag = (global_rank * 10000 + i) * 10 + ALLTOALL_TAG;
#ifdef DEBUG_PRINT
