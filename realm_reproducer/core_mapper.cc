...
class CoreMapper : public Legion::Mapping::NullMapper {
 public:
  CoreMapper(MapperRuntime* runtime, Machine machine);
...
CoreMapper::CoreMapper(MapperRuntime* rt, Machine m)
  : NullMapper(rt, m),
                                         ...
                                         SelectShardingFunctorOutput& output)
{
  ...
  assert(launch_dim == 1);
  output.chosen_functor = 65536 + LEGATE_CORE_TOPLEVEL_TASK_SHARD_ID;
}
...
void register_legate_core_mapper(Machine machine, Runtime* runtime, const std::set<Processor> &local_procs)
{
  ...
  {
    runtime->replace_default_mapper(new CoreMapper(runtime->get_mapper_runtime(), machine), *it);
  }
}
