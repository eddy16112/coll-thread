                   ...
                   const int* mapping_table)
{
  ...
  if (global_comm->global_rank == 0) {
    ...
    thread_comms[global_comm->unique_id].buffers = (const void**)malloc(sizeof(void*) * global_comm_size);
    thread_comms[global_comm->unique_id].displs  = (const int**)malloc(sizeof(int*) * global_comm_size);
